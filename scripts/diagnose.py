...
for iA in xrange(2,len(argv)):
    if 'verbose' in argv[iA]:
        verbose = ('true' in argv[iA].split('=')[-1].lower())
    if 'debug' in argv[iA]:
        debug = ('true' in argv[iA].split('=')[-1].lower())
...
for dsRaw in datasets:
    ...
    REMOVABLETHRESHOLD =  0.01
    if len(dsInstance.stuckFiles) <= REMOVABLETHRESHOLD * dsInstance.nFiles:
